...
const getEnvironment = () => {
  ...
  hdrPromise = new Promise(async (resolve, reject) => {
    if (!renderer || !scene) return reject();
    const pmremGenerator = new THREE.PMREMGenerator(renderer);
    try {
      const loader = new HDRLoader();
      const texture = await loader.loadAsync(
        `${import.meta.env.BASE_URL || "/"}threejs/textures/env-map-2-2.hdr`,
      );
...
onMounted(async () => {
  ...
  resizeObserver = new ResizeObserver(() => refreshDimensions());
  ...
  <div class="viewer-3d-container" ref="containerRef">
    ...
    <div v-if="hasError" class="error-overlay">
      <div class="error-content">
        ...
        <button type="button" class="retry-btn" @click="reloadScene">
          Try Again
        </button>
      </div>
    </div>
